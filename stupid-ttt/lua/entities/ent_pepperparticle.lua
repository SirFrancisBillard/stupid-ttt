...
if SERVER then
	...
	hook.Add("RenderScreenspaceEffects", "PepperSprayBlindHandler", function()
		...
		if ply:GetNWBool("IsPeppered") and ply:GetNWInt("PepperAmount") > 0 then
...
function ENT:DrawTranslucent()
	if CLIENT then
		...
		cam.Start3D(EyePos(),EyeAngles()) -- Start the 3D function so we can draw onto the screen.
			...
			render.DrawSprite( self:GetPos(), 50, 50, Color(r,g,b,a)) -- Draw the sprite in the middle of the map, at 16x16 in it's original colour with full alpha.
		cam.End3D()
	end
end
